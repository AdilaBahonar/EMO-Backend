...
*/                if (BuisnessAdmins.Any())
        ...
        public async Task<ResponseModel<List<UserResponseDTO>>> GetBusinessAdminsByBusinessId(string businessId)
        {
            try
            {
                ...
                var BuisnessAdmins = await db.tbl_user.Include(x=>x.sub_user_type).Include(x=>x.gender).Include(x=>x.user_image).Where(x => x.fk_business == Guid.Parse(businessId) && x.sub_user_type.user_type.user_type_name == "Business Admin").ToListAsync();
